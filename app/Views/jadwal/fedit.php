...
<div class="modal fade text-left" id="editJadwal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel160" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title white" id="myModalLabel160">Form Edit Data Jadwal</h5>
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <i data-feather="x"></i>
                </button>
            </div>
            <form id="updateJadwal" action="/jadwal/update" method="post" class="form form-horizontal">
                        ...
                        <div class="col-md-8">
                            <select class="form-control" id="mataPelajaran" name="mata_pelajaran_id" required>
                                ...
                                <?php foreach ($mataPelajaran as $mp): ?>
                                    <option value="<?= $mp->id ?>" <?= $jadwal->mata_pelajaran_id == $mp->id ? 'selected' : '' ?>>
                                        <?= $mp->nama ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-md-8">
                            <select class="form-control" name="hari" required>
                                ...
                                <?php foreach ($hariOptions as $key => $value): ?>
                                    <option value="<?= $key ?>" <?= $jadwal->hari == $key ? 'selected' : '' ?>>
                                        <?= $value ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
...
<script>
    $(document).ready(function() {
        // Ambil ID Mata Pelajaran yang sudah dipilih dari form
        var mataPelajaranId = $('#mataPelajaran').val();
        
        // Mengisi dropdown guru saat form di-load
        if (mataPelajaranId) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        $.each(response.guru, function(index, guru) {
                            guruOptions += '<option value="' + guru.id + '"' + (guru.id == '<?= $jadwal->guru_id ?>' ? ' selected' : '') + '>' + guru.nama + '</option>';
                        });
